...
pthread_cond_t cond_empty_pub = PTHREAD_COND_INITIALIZER;
pthread_cond_t cond_full_pub = PTHREAD_COND_INITIALIZER;
...
command_t *publish_buffer[BABBLE_PRODCONS_SIZE];

int buf_count , buf_answer_count, buf_publish_count;
...
void * comm_routine(int vargp){
  ...
  while((recv_size=network_recv(newsockfd, (void**) &recv_buff)) > 0){
      ...
      if(cmd->cid == 1)
      {
          while(buf_publish_count == BABBLE_PRODCONS_SIZE){
              pthread_cond_wait(&cond_empty, &mutex);
          }
          publish_buffer[buf_publish_count] = cmd;
          buf_publish_count = buf_publish_count+1;
      }
      else
      {
          while(buf_count == BABBLE_PRODCONS_SIZE){ // if all buffer full then wait
            pthread_cond_wait(&cond_empty, &mutex);
          }
...
void * executor_routine(){
	...
	answer_t *answer=NULL;
      ...
      while(buf_publish_count == 0 && buf_count == 0)
      {
          pthread_cond_wait(&cond_full, &mutex);
      }
      if (buf_publish_count != 0)
      {
          buf_publish_count--;
          cmd = publish_buffer[buf_publish_count];
      }
      else
      {
          buf_count--;
